...
add_camera_mute_feature() {
    ...
    <CscFeature_Camera_ConfigEnableCameraMute>TRUE</CscFeature_Camera_ConfigEnableCameraMute>')
        ...
        if $modified; then
            echo "$content" >"$file"
            echo "Camera mute feature has been added."
        fi
    done
...
add_call_recording_feature() {
    ...
    <CscFeature_Camera_EnableCameraDuringCall>TRUE</CscFeature_Camera_EnableCameraDuringCall>')
        ...
        if $modified; then
            echo "$content" >"$file"
            echo "Call recording feature has been added."
        fi
    done
    encode_csc >/dev/null 2>&1
}
...
add_network_speed_feature() {
    ...
    <CscFeature_Setting_SupportRealTimeNetworkSpeed>TRUE</CscFeature_Setting_SupportRealTimeNetworkSpeed>')
        echo "$content" >"$file"
        echo "Network speed display feature has been added."
    done
    encode_csc >/dev/null 2>&1
}
...
advanced_material_support() {
    while IFS= read -r -d '' file; do
        ...
        if [[ "$parent_dir" == "system" ]]; then
            ...
            echo "Advanced material support has been added: $file"
        fi
    ...
    while IFS= read -r -d '' file; do
        ...
        if [[ "$parent_dir" == "vendor" ]]; then
            if ! grep -q "^ro.vendor.se.type=HCE,UICC,eSE$" "$file"; then
                ...
                echo "Xiaomi smart card support has been added: $file"
            else
                echo "Target line already exists in the file: $file"
            fi
        fi
    done < <(find "$onepath" -type f -name "build.prop" -print0)
}
...
decode_csc() {
    ...
    for file in "${files[@]}"; do
        while IFS= read -r -d '' filepath; do
            ...
            if [ $? -eq 0 ]; then
                echo "Decoding of $file completed."
                rm "$filepath"
            fi
...
deodex() {
    ...
    if [ "$found" = false ]; then
        echo "No files related to odex available for removal."
    fi
}
...
deodex_key_files() {
    ...
    if [ "$found" = false ]; then
        echo "No related files available for removal."
    fi
}
